...
| 2 | Freeze the question, hypotheses, claims that would and would not be justified, primary metric, layer band rule, and success/null criteria. | A timestamped preregistration. **Pass:** a null result would still produce a clear conclusion. | Narrow the claim to the final-token residual stream; do not broaden the experiment. |
...
| 7 | Validate the pipeline using controls on development data: relation deletion, question truncation, and label permutation. Freeze the analysis script. | Control table and signed analysis freeze note. **Pass:** the official smoke test still works and at least one semantic control changes the readout as predicted, or the lack of change is logged as a warning. | Fix extraction bugs only. If controls remain null but the official example passes, proceed—the method may genuinely lack binding selectivity. |
